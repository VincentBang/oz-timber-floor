...
const CATEGORY_META = {
  hybrid: {
    ...
    heroImage: "/assets/images/products/hybrid/topdeck-lumiere-ultra-hd/lumiere-lovanna-oak.webp"
  },
  laminate: {
    label: "Laminate",
    ...
    heroImage: "/assets/images/products/laminate/topdeck-pantora-amor/amor-amalfi-oak.webp"
  },
  engineered: {
    ...
    heroImage: "/assets/images/products/engineered/topdeck-project-oak/project-oak-baltimore-oak.webp"
  },
  solid: {
    label: "Solid timber",
...
const RANGE_OVERRIDES = {
  ...
  "hybrid-topdeck-lumiere-ultra-hd": {
  ...
  "laminate-topdeck-pantora-lifestyle": {
  ...
  "laminate-topdeck-prime-contemporary-plus": {
  ...
  "engineered-topdeck-wooden-land-foreign-species": {
  ...
  "engineered-topdeck-wooden-land-australian-136mm": {
  ...
  "engineered-topdeck-wooden-land-australian-190mm": {
  ...
  "engineered-topdeck-wooden-land-herringbone": {
  ...
  "engineered-topdeck-project-oak": {
    slug: "project-oak",
  ...
  "engineered-topdeck-castel-nuovo-herringbone": {
  ...
  "engineered-topdeck-lavanda-oak": {
    slug: "lavanda-oak",
  ...
  "engineered-topdeck-cavallo-bianco-chevron": {
  ...
  "solid-topdeck-raw": {
    slug: "topdeck-raw-solid-timber",
    shortLabel: "Topdeck Raw Solid Timber",
    brandFamily: "Topdeck Solid Timber",
    liveBlurb: "Unfinished solid hardwood ready for custom staining, sanding, polishing or coating to suit the project."
  },
  "solid-topdeck-pre-finished": {
    slug: "topdeck-pre-finished-solid-timber",
    shortLabel: "Topdeck Pre-Finished Solid Timber",
    brandFamily: "Topdeck Solid Timber",
...
const TOPDECK_RANGE_IDS = new Set(Object.keys(RANGE_OVERRIDES).filter((key) => key !== "solid-topdeck-raw" && key !== "solid-topdeck-pre-finished"));
const CTA_LABELS = [
...
function getRangeFamily(rangeId) {
  return RANGE_OVERRIDES[rangeId]?.brandFamily || "Topdeck Flooring";
}
...
function inferWarranty(categoryKey) {
  if (categoryKey === "hybrid") return "Confirm the current Topdeck residential and commercial warranty for the exact hybrid collection before ordering.";
  if (categoryKey === "laminate") return "Confirm the current Topdeck laminate warranty for the selected collection, room type and installation method before ordering.";
  if (categoryKey === "engineered") return "Confirm the current Topdeck engineered timber warranty, maintenance requirements and site conditions before ordering.";
  return "Confirm the current Topdeck solid timber warranty and finishing requirements before ordering.";
}
  ...
  for (const categoryKey of ["hybrid", "laminate", "engineered"]) {
    for (const product of api.getProductsByCategory(categoryKey)) {
      ...
      items.push({
        ...
        no_checkout: true,
        source: "Topdeck Flooring live-aligned import"
      });
...
function createSolidRanges() {
  return [
    {
      range_id: "solid-topdeck-raw",
      slug: getRangeSlug("solid-topdeck-raw"),
      range_name: "Topdeck Raw Solid Timber",
      range_short_label: "Raw Solid Timber",
      family: "Topdeck Solid Timber",
      category_key: "solid",
      category: "Solid timber",
      url: `/ranges/${getRangeSlug("solid-topdeck-raw")}/`,
      description: [
      ...
      technical: [
        { label: "Product family", value: "Topdeck Solid Timber" },
      ...
      productCount: 0,
      liveBlurb: RANGE_OVERRIDES["solid-topdeck-raw"].liveBlurb,
      image: CATEGORY_META.solid.heroImage
    },
    {
      range_id: "solid-topdeck-pre-finished",
      slug: getRangeSlug("solid-topdeck-pre-finished"),
      range_name: "Topdeck Pre-Finished Solid Timber",
      range_short_label: "Pre-Finished Solid Timber",
      family: "Topdeck Solid Timber",
      category_key: "solid",
      category: "Solid timber",
      url: `/ranges/${getRangeSlug("solid-topdeck-pre-finished")}/`,
      description: [
      ...
      productCount: 0,
      liveBlurb: RANGE_OVERRIDES["solid-topdeck-pre-finished"].liveBlurb,
      image: CATEGORY_META.solid.heroImage
    }
  ];
}
  ...
  <body${bodyClass ? ` class="${bodyClass}"` : ""}>
    ...
    <footer class="site-footer">
      <div class="shell footer-grid">
        ...
        <div><h3>Products</h3><div class="footer-links"><a href="/laminate-flooring-sydney/">Laminate</a><a href="/hybrid-flooring-sydney/">Hybrid</a><a href="/engineered-timber-flooring-sydney/">Engineered timber</a><a href="/solid-timber-flooring-sydney/">Solid timber</a><a href="/vinyl-flooring-sydney/">Vinyl</a><a href="/bamboo-flooring-sydney/">Bamboo</a></div></div>
...
function updateCategoryPage(filePath, categoryLabel, topdeckRanges, topdeckProducts) {
  let html = fs.readFileSync(filePath, "utf8");
  const rangeSection = `<section class="section soft"><div class="shell"><div class="section-head"><div><p class="eyebrow">Topdeck ranges</p><h2>Imported Topdeck ${escapeHtml(categoryLabel.toLowerCase())} collections</h2></div><p>These collections follow the current supplier range structure, with each range leading to supply price, stock and supply + install enquiries.</p></div><div class="grid-3">${topdeckRanges.map((range) => `<a class="category-card" href="${escapeHtml(range.url)}"><strong>${escapeHtml(range.range_short_label)}</strong><p>${escapeHtml(range.liveBlurb || plainTextList(range.description || []).slice(0, 180))}</p></a>`).join("")}</div></div></section>`;
  const productSection = `<section class="section catalogue-products"><div class="shell"><div class="section-head"><div><p class="eyebrow">Topdeck products</p><h2>Current Topdeck products in this category</h2></div><p>Use these product pages to ask about current availability, supply-only pricing, floor preparation and supply + install scope.</p></div><div class="grid-3">${productCards(topdeckProducts.slice(0, 12))}</div></div></section>`;
  html = replaceOrInsertSection(html, `TOPDECK ${categoryLabel.toUpperCase()} RANGES`, rangeSection, /<section class="section">\s*<div class="shell cta">/);
  html = replaceOrInsertSection(html, `TOPDECK ${categoryLabel.toUpperCase()} PRODUCTS`, productSection, /<section class="section">\s*<div class="shell cta">/);
  fs.writeFileSync(filePath, html);
}

function updateProductsPage(filePath, ranges, products) {
  let html = fs.readFileSync(filePath, "utf8");
  const rangeSection = `<section class="section soft"><div class="shell"><div class="section-head"><div><p class="eyebrow">Topdeck supplier import</p><h2>Imported Topdeck ranges now available in Oz</h2></div><p>Oz now keeps the current Topdeck range structure across solid timber, engineered timber, laminate and hybrid pathways instead of flattening those searches into a single generic product page.</p></div><div class="grid-4">${ranges.map((range) => `<a class="category-card" href="${escapeHtml(range.url)}"><strong>${escapeHtml(range.range_short_label)}</strong><p>${escapeHtml(range.category)} · ${escapeHtml(range.family)}</p></a>`).join("")}</div></div></section>`;
  const productSection = `<section class="section catalogue-products"><div class="shell"><div class="section-head"><div><p class="eyebrow">Topdeck featured products</p><h2>Current Topdeck catalogue highlights</h2></div><p>These product pages are built for catalogue-commerce only, with no checkout and no instant flooring price calculation.</p></div><div class="grid-3">${productCards(products.slice(0, 18))}</div></div></section>`;
  html = replaceOrInsertSection(html, "TOPDECK PRODUCTS PAGE RANGES", rangeSection, /<section class="section"><div class="shell cta">/);
  html = replaceOrInsertSection(html, "TOPDECK PRODUCTS PAGE PRODUCTS", productSection, /<section class="section"><div class="shell cta">/);
  fs.writeFileSync(filePath, html);
}
...
function updateProductCatalogue(products) {
  ...
  const nonTopdeck = (current.products || []).filter((item) => item.supplier !== "Topdeck Flooring");
  const merged = nonTopdeck.concat(products.map((product) => ({
    url: product.url,
    name: product.name,
    brand: product.brand,
...
function updateDataGaps(products, ranges) {
  ...
  const topdeckRangeCount = ranges.length;
  const topdeckProductCount = products.length;
  const content = `# Catalogue-commerce data gaps

- Topdeck import completed for ${topdeckRangeCount} ranges and ${topdeckProductCount} products using the current supplier range structure as at May 14, 2026.
- Imported categories: hybrid, laminate, engineered timber, plus range-level solid timber pathways.
- Product imagery was copied from the existing Topdeck WebP catalogue assets already held in the shared workspace.

Still needing supplier-confirmed data before final launch:

- Product-level Topdeck solid timber SKUs, board sizes and finished colour lists for raw and pre-finished solid timber.
...
- Current stock flags and discontinued colour confirmation from Topdeck.
...
function main() {
  ...
  const products = createTopdeckProducts(api);
  const sourceProductsById = new Map();
  for (const categoryKey of ["hybrid", "laminate", "engineered"]) {
    for (const product of api.getProductsByCategory(categoryKey)) {
      if (TOPDECK_RANGE_IDS.has(product.rangeId)) {
