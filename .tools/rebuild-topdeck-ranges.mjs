...
const WARRANTY_BOOKLET = "https://cdn.shopify.com/s/files/1/0739/9132/3868/files/TDF202510_-_WARRANTY_BOOKLET.pdf?v=1759806811";
const HYBRID_BROCHURE = "https://cdn.shopify.com/s/files/1/0739/9132/3868/files/TDF202510_-_HYBRID_PLANKS.pdf?v=1759819781";
...
const artisanConfig = {
  ...
  faq: [
    ...
    ["Are ratings and certification documents available?", "Supplier documents list fire, slip, acoustic and VOC or environment certificates. Suitability for a specific project should be confirmed before ordering."],
...
function docsForRange(range) {
  if (range.slug === "artisan-tile") return artisanConfig.docs;
  const docs = [];
  if (range.sourceUrl) {
    docs.push([
  ...
  if (range.category === "Hybrid") {
    docs.push([
      "Hybrid brochure",
      "Review Topdeck's hybrid planks brochure before comparing colours, construction and installation planning.",
      HYBRID_BROCHURE,
    ]);
  }
  docs.push([
    "Warranty booklet",
    "Review the current supplier warranty conditions before final product selection, room-use decisions or installation planning.",
    WARRANTY_BOOKLET,
  ]);
  return docs;
}
...
function faqForRange(range) {
  ...
  return [
    ...
    [`Are supplier documents available for ${range.name}?`, `Yes. Use the supplier reference and warranty documents on this page, and ask Oz Timber Floor to confirm which product documents are relevant to your project.`],
...
function relatedRanges(range, topdeckRanges) {
  return topdeckRanges
...
function buildPage(range, topdeckRanges) {
  ...
  const related = relatedRanges(range, topdeckRanges);
...
const topdeckRanges = ranges.filter((range) => range.supplier === "Topdeck Flooring" && range.publicCatalogueStatus !== "legacy-hidden");

for (const range of topdeckRanges) {
  const dir = path.join(root, range.url.replace(/^\/|\/$/g, ""));
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "index.html"), buildPage(range, topdeckRanges));
}

console.log(`Rebuilt ${topdeckRanges.length} Topdeck range pages`);
