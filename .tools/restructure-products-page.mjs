...
const categoryCards = [
  {
    ...
    image: "/assets/products/hybrid/avala/avala-blackbutt.webp",
    alt: "Avala Blackbutt hybrid flooring colour swatch",
  ...
  },
  {
    ...
    image: "/assets/products/laminate/prime-legend/legend-atlantic-oak.webp",
    alt: "Prime Legend Atlantic Oak laminate flooring colour swatch",
  ...
  },
  {
    ...
    image: "/assets/products/engineered-timber/botanica/alaska.jpg",
    alt: "Botanica Alaska engineered timber flooring colour swatch",
  ...
  },
  {
    ...
    alt: "Solid timber flooring category sample",
  ...
  },
  {
    ...
    image: "/assets/images/categories/vinyl-flooring-thumbnail.webp",
    alt: "Vinyl flooring category sample",
...
function categoryCard(card) {
  return `<a class="category-card chooser-card" href="${esc(card.href)}"><img class="category-thumb" src="${esc(card.image)}" alt="${esc(card.alt)}" loading="lazy" decoding="async"><span><strong>${esc(card.name)}</strong><p>${esc(card.text)}</p><em>View category</em></span></a>`;
}

function projectNeedCard([title, text, href]) {
  return `<article class="category-card"><strong>${esc(title)}</strong><p>${esc(text)}</p><a class="card-link" href="${esc(href)}">View pathway</a></article>`;
}
...
function rangeCard(range) {
  ...
  const pills = [range.category, range.thickness, range.colourCount ? `${range.colourCount} colours` : ""]
    ...
    .slice(0, 3);
  return `<article class="card range-summary-card" data-range-card data-category="${esc(range.category)}">${image ? `<img class="category-thumb" src="${esc(image)}" alt="${esc(`${range.name} ${String(range.category).toLowerCase()} range colour swatch`)}" loading="lazy" decoding="async">` : ""}<p class="catalogue-overline">${esc(range.category || "Flooring range")}</p><h3>${esc(range.name)}</h3><p>${esc(rangeDescriptions[range.id] || range.shortDescription || "Compare colours, specifications, stock availability and project fit for this flooring range.")}</p><p class="product-meta">${pills.map((pill) => `<span class="pill">${esc(pill)}</span>`).join("")}</p><a class="card-link" href="${esc(range.url)}">View range</a></article>`;
}
