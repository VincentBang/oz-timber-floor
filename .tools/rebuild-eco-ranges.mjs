...
const ECO_SUPPLIER_HOME = "https://ecoflooring.com.au/";
...
function fitCards(range) {
  const set = range.category === "Hybrid"
      ? [
...
const ecoRanges = ranges.filter((range) => range.supplier === "Eco Flooring Systems" && range.publicCatalogueStatus !== "legacy-hidden");

for (const range of ecoRanges) {
  const dir = path.join(root, range.url.replace(/^\/|\/$/g, ""));
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "index.html"), buildPage(range, ecoRanges));
}

console.log(`Rebuilt ${ecoRanges.length} Eco Flooring range pages`);
