...
function rangeCard(range) {
  ...
  return `<article class="card range-summary-card">${img ? `<img class="category-thumb" src="${esc(img)}" alt="${esc(`${range.name} ${String(range.category).toLowerCase()} colour swatch`)}" loading="lazy" decoding="async">` : ""}<p class="catalogue-overline">${esc(range.category || "Flooring range")}</p><h3>${esc(range.name)}</h3><p>${esc(range.shortDescription || categoryCopy[range.category]?.hero(range) || "Compare colours, specifications, stock availability and project fit for this flooring range.")}</p><p class="product-meta">${pills.map((pill) => `<span class="pill">${esc(pill)}</span>`).join("")}</p><a class="card-link" href="${esc(range.url)}">View range</a></article>`;
}
