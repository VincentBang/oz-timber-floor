...
const PREFERENCE_SUPPLIER_HOME = "https://preferencefloors.com.au/";
const WARRANTY_BOOKLET = PREFERENCE_SUPPLIER_HOME;
const HYBRID_BROCHURE = PREFERENCE_SUPPLIER_HOME;
...
function productListForRange(range) {
  ...
  return [...direct, ...indirect].filter((product) => {
    if (product.publicCatalogueStatus === "legacy-hidden") return false;
...
function descriptionForRange(range) {
  ...
  const name = String(range.name || "").trim();
  if (name.toLowerCase().includes(category.toLowerCase())) {
    return `Browse ${name} colours, current product details and stock enquiries in Sydney.`;
  }
  return `Browse ${name} ${category} colours, current product details and stock enquiries in Sydney.`;
}
...
function heroLead(range) {
  ...
  if (range.category === "Laminate") {
    return `${range.name} is a laminate flooring range for customers comparing dry-area timber-look floors, current product information and installation planning before ordering.`;
  }
    ...
    : range.category === "Hybrid"
      ...
      : range.category === "Laminate"
        ? [
            ...
            ["Supply-only enquiries", "Useful when quantities, trims and current product details should be confirmed first."],
...
function faqForRange(range) {
  ...
  return [
    ...
    [`Can Oz Timber Floor confirm product information for ${range.name}?`, `Yes. Oz Timber Floor can help confirm current product information, stock timing, preparation notes and any relevant project requirements before ordering.`],
...
const preferenceRanges = ranges.filter((range) => range.supplier === "Preference Floors" && range.publicCatalogueStatus !== "legacy-hidden");

for (const range of preferenceRanges) {
  const dir = path.join(root, range.url.replace(/^\/|\/$/g, ""));
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "index.html"), buildPage(range, preferenceRanges));
}

console.log(`Rebuilt ${preferenceRanges.length} Preference Floors range pages`);
