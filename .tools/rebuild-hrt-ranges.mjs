...
const HRT_SUPPLIER_HOME = "https://hrttimberflooring.com.au/";
const WARRANTY_BOOKLET = HRT_SUPPLIER_HOME;
const HYBRID_BROCHURE = HRT_SUPPLIER_HOME;
...
const hrtRanges = ranges.filter((range) => range.supplier === "HRT Timber Flooring" && range.publicCatalogueStatus !== "legacy-hidden");

for (const range of hrtRanges) {
  const dir = path.join(root, range.url.replace(/^\/|\/$/g, ""));
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "index.html"), buildPage(range, hrtRanges));
}

console.log(`Rebuilt ${hrtRanges.length} HRT range pages`);
